...
Completa el comando para **calcular el promedio de la columna 3** de `datos.txt`:
...
- `NR` es una variable especial de `awk` con el número total de registros (líneas)\
...
Completa el pipeline para encontrar **las 5 secuencias más frecuentes** en la columna 3 de `reads_summary.tsv`:
...
**Desglose:** - `cut -f3`: extrae la columna 3 - `sort`: ordena para que `uniq` funcione correctamente - `uniq -c`: cuenta ocurrencias, prepende el conteo - `sort -k1nr`: ordena numéricamente en reversa por la columna 1 (conteo) - `head -n 5`: muestra los 5 más frecuentes
:::
