...
La idea fundamental es pensar en las E-utilities como piezas de un
sistema de consulta:
...
Esta separación permite construir pipelines reproducibles y evita
descargar información que no necesitamos.
...
Antes de hacer una búsqueda compleja, es útil saber cómo está organizada
la base de datos. `einfo` proporciona información sobre las bases
disponibles, los campos que pueden utilizarse en las consultas y los
enlaces entre bases.

Esto es especialmente importante porque las búsquedas de NCBI no son
simplemente búsquedas de texto libre. Los campos como `[Organism]`,
`[Gene Name]`, `[PDAT]` o `[Title/Abstract]` permiten restringir la
consulta y obtener resultados mucho más precisos.
...
Esta separación permite, por ejemplo, buscar todos los transcritos que
cumplen una condición y después decidir si queremos sus secuencias
FASTA, registros GenBank o metadatos.
...
Por ejemplo, podemos partir de una publicación, un gen o un registro de
SRA y utilizar sus relaciones para encontrar información adicional. Esto
evita tener que buscar manualmente cada identificador.
:::
...
`xtract` permite seleccionar patrones y elementos específicos dentro de
ese XML y convertirlos en una salida tabular. Es especialmente útil
cuando queremos pasar de una respuesta de NCBI a un archivo TSV que
posteriormente pueda analizarse con `awk`, `cut`, R o Python.
...
**¿Cuál pipeline descarga correctamente las secuencias de ARNm de BRCA2
humano en formato FASTA?**
...
-   **a)** `efetch` no tiene flag `--query`; requiere IDs específicos o
    input desde `esearch`
-   **b)** Busca en la base de datos `gene`, no en `nucleotide`, y sin
    filtro de mRNA puede traer otros tipos de secuencias
-   **c)** ✅ Pipeline correcto: `esearch` busca con filtros apropiados
    → `efetch` descarga en FASTA
-   **d)** `einfo` sirve para explorar la estructura de la base de
    datos, no para buscar secuencias
:::
...
Completa el pipeline para **descargar los metadatos de runs del SRA**
asociados al BioProject `PRJNA548382` y extraer: Run ID, número de
bases, y nombre del organismo:
...
1.  Busque en PubMed artículos sobre "single-cell RNA-seq" publicados en
    2022-2023
...
Usando E-utilities, realiza un análisis del número de publicaciones por
año (2018-2023) sobre los términos **"metagenomics"**,
**"transcriptomics"** y **"proteomics"** y genera una tabla comparativa:
